...
BenchResult run_benchmark(const OpDesc& desc) {
    ...
    } else if (desc.backend == "approach4") {
#ifdef ENABLE_ZENDNN
        if (desc.op_name == "matmul") {
            return bench_matmul_approach4(desc);
        } else if (desc.op_name == "matmul_id") {
            return bench_matmul_id_approach4(desc);
        } else {
            fprintf(stderr, "error: approach4 backend only supports matmul and matmul_id\n");
            exit(1);
        }
#else
        fprintf(stderr,
                "error: approach4 requires ZenDNN. "
                "Rebuild with -DENABLE_ZENDNN=ON\n");
        exit(1);
#endif
    } else {
...
void print_results(const OpDesc& desc, const BenchResult& result) {
    ...
    printf("%-8s %-6s %-6s %-6s %-6s %-15s %-15s %-8s %-18s %-20s %-20s %-18s %-10s\n",
           ...
           "Avg_total(ms)", "Avg_ctx_init(ms)", "Avg_op_setup(ms)",
           "Avg_exec(ms)", "GFLOPS");
    ...
    printf("%-8s %-6d %-6d %-6d %-6d %-15s %-15s %-8d "
           "%-18.2f %-20.2f %-20.2f %-18.2f %-18.2f\n",
           desc.backend.c_str(),
